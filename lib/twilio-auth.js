...
async function assertCanDial(req, res) {
  try {
    ...
    if (!prof || prof.role !== 'master') return true; // agents always dial
    ...
    if (!org || org.plan === 'internal') return true;

    res.status(403).json({
      error: 'The owner account is for managing your team and does not include a dialing seat. '
           + 'Add yourself as a user from the Dashboard if you want to dial.',
