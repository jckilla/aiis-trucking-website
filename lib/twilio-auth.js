...
const ALLOWED_ORIGINS = [
  // Aduna Dialer customers. Same app, its own brand - a customer who bought
  // Aduna Dialer should never be sent to an insurance company's domain.
  'https://app.adunadialer.com',
  // AIIS's own workspace.
...
async function getCallerIdForRequest(req, fallbackNumber) {
  try {
    ...
    const valid = (n) => /^\+1[2-9]\d{9}$/.test(String(n || '').trim());

    // 1) The rep's own caller ID wins - local presence is set per person.
    const { data: prof } = await sb
      .from('profiles')
      .select('twilio_caller_id, org_id')
      .eq('id', u.user.id)
      .maybeSingle();
    const cid = prof && prof.twilio_caller_id ? String(prof.twilio_caller_id).trim() : '';
    if (valid(cid)) return cid;

    // 2) Otherwise the WORKSPACE's own number. This is what keeps one
    //    customer's caller reputation separate from another's while they all
    //    share a single Twilio account: credentials stay constant, only the
    //    number varies. Per-customer subaccounts can come later without
    //    changing this precedence.
    if (prof && prof.org_id) {
      const { data: org } = await sb
        ...
        .eq('id', prof.org_id)
        .maybeSingle();
      const orgNum = org && org.twilio_number ? String(org.twilio_number).trim() : '';
      if (valid(orgNum)) return orgNum;
    }

    // 3) Fall back to the deployment's default number (AIIS's shared number).
...
/**
 * The org the caller belongs to, or null. Used to pick per-tenant Twilio
 * credentials; callers must treat null as "use the deployment defaults".
 */
async function getCallerOrgId(req) {
  try {
    const user = await getSessionUser(req);
    if (!user) return null;
    const sb = createClient(SUPABASE_URL, SUPABASE_KEY, { auth: { persistSession: false, autoRefreshToken: false } });
    const { data: prof } = await sb.from('profiles').select('org_id').eq('id', user.id).maybeSingle();
    return (prof && prof.org_id) || null;
  } catch (e) {
    return null;
  }
}
...
async function assertCanDial(req, res) {
  try {
    ...
    const { data: org } = await sb
      .from('orgs')
      .select('plan, active, telephony_status')
      .eq('id', prof.org_id)
      .maybeSingle();

    if (!org) return true;

    // Per-tenant telephony. Without its own Twilio subaccount a workspace would
    // dial on the shared account and inherit someone else's number reputation -
    // a billing leak and a compliance entanglement between unrelated
    // businesses. Fail CLOSED here: an unprovisioned tenant must not dial.
    if (org.plan !== 'internal' && org.telephony_status !== 'ready') {
      res.status(403).json({
        error: 'Your phone system is still being set up. We are provisioning your own '
             + 'number and calling account - you will be able to dial as soon as that is done.',
...
module.exports = { setCorsHeaders, verifyRequest, verifySession, getSessionUser, getCallerOrgId, requireMaster, assertCanDial, getCallerIdForRequest, ALLOWED_ORIGINS };
